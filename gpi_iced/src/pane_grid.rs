...
impl Example {
    fn new() -> Self {
        let (panes, _) = pane_grid::State::new(MyPane::new(0));
    ...
    fn update(&mut self, message: Message) {
        match message {
            Message::Split(axis, pane) => {
                let result = self
                    .panes
                    .split(axis, pane, MyPane::new(self.panes_created));
            ...
            Message::Dragged(pane_grid::DragEvent::Dropped { pane, target }) => {
                println!("dropped! {pane:?} ,{target:?}");
                self.panes.drop(pane, target);
            }
            Message::Dragged(other) => {
                println!("drag event: {other:?}");
            }
            Message::TogglePin(pane) => {
                if let Some(MyPane { is_pinned, .. }) = self.panes.get_mut(pane) {
                    *is_pinned = !*is_pinned;
                }
            }
            ...
            Message::CloseFocused => {
                if let Some(pane) = self.focus {
                    if let Some(MyPane { is_pinned, .. }) = self.panes.get(pane) {
                        if !is_pinned {
                            if let Some((_, sibling)) = self.panes.close(pane) {
                                self.focus = Some(sibling);
                            }
