...
where
    S: Clone,
{
    nodes: ordermap::OrderMap<NodeIndex, T>,
    ...
    S: Clone,
{
    ...
    pub fn nodes_ref(&self) -> &OrderMap<NodeIndex, T> {
        &self.nodes
    }

    ///Get a mutable reference  to a node value at a given index
    ///panics if index is not valid
    pub fn get_mut_node(&mut self, nx: NodeIndex) -> &mut T {
...
mod test {
    ...
    fn vertex() {
        ...
        let mut g: Graph<Node, &str> = Graph::new();
